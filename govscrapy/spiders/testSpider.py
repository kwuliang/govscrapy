...
class GovSpider(CrawlSpider):
    ...
    allowed_domains = ['www.junz.org']
    start_urls = ['https://www.junz.org/about/']
...
#解析页面
    ...
    def parse(self, response):
        ...
        #     xls = response.selector.xpath("//a/@href[contains(.,'.xls')]").extract() + response.selector.xpath("//a/@href[contains(.,'.xlsx')]").extract()
        #     doc = response.selector.xpath("//a/@href[contains(.,'.doc')]").extract() + response.selector.xpath("//a/@href[contains(.,'.docx')]").extract()
        #     # urls = images + pdf + xls
        #     # ahref = response.selector.xpath("//@href").extract()
        #
        #     # ahref = self.link_extractor.extract_links(response.body)
        #     #
        #     urls =  images + pdf + xls + doc
